...
def cbfx(pvname=None,value=None,char_value=None,type=None,enum_strs=None,**kw):
    if indeadband(float(tar[0][0]),float(value),dbd*xdbdf)==1:
        tar[0][1] = 0
    else:
        tar[0][1] = 1
def cbfy(pvname=None,value=None,char_value=None,type=None,enum_strs=None,**kw):
    if indeadband(float(tar[1][0]),float(value),dbd*ydbdf)==1:
        tar[1][1] = 0
    else:
        tar[1][1] = 1
...
#####modified for HR coarse Stages; X=Attocube ECS505, Y=MicronixY
    ...
    ymot=PV('XF:05IDD-ES:1{Stg:Mcrnx-Ax:Y}Mtr.VAL',connection_timeout=2)
    ymot_cur=PV('XF:05IDD-ES:1{Stg:Mcrnx-Ax:Y}Mtr.RBV',connection_timeout=2)  
