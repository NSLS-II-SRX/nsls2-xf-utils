...
def main(argv=None):
    ...
    if sys.argv[0][0]=='.':
        out_filename=filedir+repr(D0)+'/'+repr(D1)+'/'+repr(D2)+'/'+'log_'+repr(D0)+'_'+repr(D1)+'_'+repr(D2)+'_'+repr(D3)+'_'+repr(D4)+'_'+\
...
#    norm3=PV('XF:05IDD-BI:1{BPM:01}.S23',connection_timeout=2)
    ...
    str='# Bragg: %(B)6.4f ; Energy: %(E)6.4f ; WB HCEN: %(WBHC)f ; WB HSIZE: %(WBHS)f ; WB VCEN: %(WBVC)f ; WB VSIZE: %(WBVS)f'%\
     {"B":bmot_cur.get(), "E": 12398. / (2 * dSi111 * math.sin((bmot_cur.get()+dbragg)/180.*3.1416)), "WBHC":wb.hcen(), "WBHS":wb.hsize(), "WBVC":wb.vcen(), "WBVS":wb.vsize()}
    print str
    fp.write(str)
    fp.write('\n')
    str="# --------------------------------------------------------------------    "
...
#                cryocounter=0
            ...
            while (cryocounter==1):
                ...
                if (oldsig > (-1.*0.8*tavg)) and (v19st.get > 0.):
                    ...
                    cryocounter=0
            while ( options.checkbeam and (shut_open == False or current == False)):
