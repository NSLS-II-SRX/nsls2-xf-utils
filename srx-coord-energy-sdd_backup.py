...
#    H5path=fstr+repr(D0)+'/'+repr(D1)+'/'+repr(D2)+'/HDF5'
    ...
    dett.put(3)
    deti.put(float(options.acqn)*options.acqt)
    ...
    for x in range(0,pN):
        ...
        tn=time.time()-t0
        if options.sim is False:    
            str=' %(B)8.4f %(U)8.4f %(G)8.3f %(E)8.2f %(C0)10.7e %(C1)10.7e %(C2)10.7e %(C3)10.7e %(T)d %(ROI1)d %(ROI2)d %(ROI3)d %(ROI4)d %(T1)10.7e'%{"B":float(bmot_cur.get()), "U":float(umot_cur.get()), "G":float(gmot_cur.get()), "C0":nsig0, "C1":nsig1, "C2":nsig2, "C3":nsig3, "ROI1":sig0,'T':tn,"ROI2":sig1,"ROI3":sig2,"ROI4":sig3,"T1":signal0,"E":12398. / (2 * 3.1297 * math.sin((bmot_cur.get()+0.31317)/180.*3.1416))}
            print str
            fp.write(str)
            fp.write('\n')
        else:
